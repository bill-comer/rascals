...
import uk.co.utilisoft.parms.web.dto.JodaDateTimeDTO;
import uk.co.utilisoft.parms.web.dto.OptionDTO;
import uk.co.utilisoft.parms.web.dto.SearchCriteriaDTO;
import uk.co.utilisoft.parms.web.dto.SearchWrapper;
import uk.co.utilisoft.parms.web.util.QueryStringBindValueCombo;
 ...
 */
public abstract class ParmsResultsDaoHibernate<T extends SearchWrapper> extends ResultsDaoHibernate<T, Long>
{
  /**
   * {@inheritDoc}
          ...
          && currentCombo.getClassTypes() != null)
      {
        ...
        for (int innerIndex = 0; innerIndex < innerSize; innerIndex++)
        {
          ...
          currentValue = currentCombo.getValues().get(innerIndex);
            ...
            {
              String mpanCore = convertWildcards(currentValue.toString());
              aQueryToBindTo.setParameter("id" + paramCount, new MPANCore(mpanCore));
            }
            else if (currentClassType.equals(ParmsReportingPeriod.class))
            {
              aQueryToBindTo.setParameter("id" + paramCount, new ParmsReportingPeriod(
                new DateMidnight(parseTextAsJodaDateTime(currentValue.toString()))));
            }
  ...
  {
    return new ArrayList<DateTimeFormatter>();
  }
